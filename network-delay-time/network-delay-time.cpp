    ...
    int networkDelayTime(vector<vector<int>>& times, int n, int k) {
        ...
        while(!pq.empty()){
            ...
            for(auto neigh:neighs[vertex]){     //considering each neighbors
                //if time taken to reach end point(neigh.first) is less than time taken to reach starting point(i.e vertex) initially+time taken(i.e neigh.second) to reach from starting to end with another path
        ...
        return ans==INT_MAX?-1:ans;     //if it is impossible for all nodes to receive signal, i.e we still have INT_MAX in res array, so return -1. Else return ans
    }
};